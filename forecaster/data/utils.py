...
def generate_negative_samples(full_data_pdf: pd.DataFrame, num_negative_samples: int = 5, random_state: int = 42) -> pd.DataFrame:
    """
    ...
    all_user_store_pairs = pd.DataFrame(
        index=pd.MultiIndex.from_product([full_data_pdf['user_id'].unique(), full_data_pdf['store_id'].unique()],
                                         names=['user_id', 'store_id'])).reset_index()

    # Identify negative pairs by finding those not present in positive pairs
    negative_pairs = all_user_store_pairs[~all_user_store_pairs.set_index(['user_id', 'store_id']).index.isin(positive_pairs.set_index(['user_id', 'store_id']).index)]
    ...
    for user in users:
        ...
        negative_stores = negative_pairs[negative_pairs['user_id'] == user].sample(n=num_negative_samples, random_state=random_state)
        negative_samples = pd.concat([negative_samples, negative_stores])

    # Add a 'label' column indicating negative instances
    negative_samples['label'] = 0
    # Combine positive and negative samples
    balanced_dataset = pd.concat([full_data_pdf[['user_id', 'store_id', 'label']], negative_samples])
